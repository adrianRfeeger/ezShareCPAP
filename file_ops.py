...
def recursive_traversal(ezshare_instance, url, dir_path, total_files, processed_files):
    files, dirs = list_dir(ezshare_instance, url)
    processed_files = check_files(ezshare_instance, files, url, dir_path, total_files, processed_files)
    processed_files = check_dirs(ezshare_instance, dirs, url, dir_path, total_files, processed_files)
...
def check_files(ezshare_instance, files, url, dir_path: pathlib.Path, total_files, processed_files):
...
def should_download(ezshare_instance, local_path: pathlib.Path, file_ts):
...
def download_file(ezshare_instance, url, file_path: pathlib.Path, file_ts=None):
    if file_path.is_file() and not (ezshare_instance.overwrite or file_path.stat().st_mtime < file_ts) and not ezshare_instance.keep_old:
...
def check_dirs(ezshare_instance, dirs, url, dir_path: pathlib.Path, total_files, processed_files):
    for dirname, dir_url in dirs:
        new_dir_path = dir_path / dirname
        new_dir_path.mkdir(exist_ok=True)
        absolute_dir_url = urllib.parse.urljoin(url, dir_url)
        processed_files = recursive_traversal(ezshare_instance, absolute_dir_url, new_dir_path, total_files, processed_files)
    return processed_files
